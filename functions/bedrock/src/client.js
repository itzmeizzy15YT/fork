...
class Client extends Connection {
    ...
    async init() {
        if (this.serializer) return; // already init
        ...
        this.options.external?.enabled ? null : await authenticate(this, this.options)
    ...
    _connect = async () => {
        this.connection.onConnected = () => {
            ...
            this.options.external?.enabled && this.options.external?.type === 3 ?
                this.close() : this.sendLogin()
        }
    ...
    sendLogin() {
        ...
        const updPFID = (data) => btoa(atob(data).replaceAll(`aed7e8a4d485a49a-3`, `${PlayFabId}-2`));
