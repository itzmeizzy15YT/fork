...
class NethernetSignal extends EventEmitter {
    ...
    async init() {
        ...
        ws.on("open", () => this.lastLiveness = Date.now())
    ...
    async onClose(code, reason) {
        ...
        /*  1006 closure; 1011 error; 4401 unauthorized */
        const retryable = [1006, 1011, 4401, 0].includes(code)
    ...
    write(signal) {
        if (!this.ws) throw new Error('WebSocket not connected and trying to write.')
    ...
    for (const server of TurnAuthServers) {
        ...
        for (const rawUrl of server?.Urls ?? []) {
            ...
            const port = match.groups.port
                ? parseInt(match.groups.port, 10)
                : (scheme === "stuns" ? 3478 : 5349)

            if (!hostname || Number.isNaN(port)) continue

            const isTurn = scheme.startsWith("turn")

            const transport = isTurn
                ? match.groups.query
                      ?.split("&")
                      .find(param => param.startsWith("transport="))
                      ?.split("=")[1] ?? (scheme === "turns" ? "tcp" : "udp")
                : undefined

            const urlCandidates = new Set()

            const formatUrl = (scheme, port, transport) =>
                isTurn
                    ? `${scheme}:${hostname}:${port}?transport=${transport}`
                    : `${scheme}:${hostname}:${port}`

            urlCandidates.add(formatUrl(scheme, port, transport))

            if (isTurn) {
                if (transport !== "tcp") {
                    urlCandidates.add(formatUrl(scheme, port, "tcp"))
                }

                if (scheme !== "turns") {
                    urlCandidates.add(formatUrl("turns", 5349, "tcp"))
                }
            }
